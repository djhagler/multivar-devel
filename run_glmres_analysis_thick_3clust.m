indir = [pwd '/output/roi_summaries_thick_3clust'];
infix = 'thick_sm_clusters3';
indir_demog = '/space/md12/8/data/MMILDB/HAG_PING/analysis/PING/data';
instem_demog = 'PING_demog_150424';
results_outdir = [pwd '/output/output_glmres_thick_3clust'];
data_outdir = [pwd '/output/roi_summaries_thick_3clust_resid'];
merge_field = 'VisitID';
...
for f=1:length(flist)
  ...
  if ~exist(fname_resid,'file') || forceflag
    ...
    measlist_infix = cellfun(@(x) sprintf('%s_%s',x,infix),measlist,'UniformOutput',false);
    ind_meas = find(~cellfun(@isempty,regexp(fstem,measlist_infix)));
