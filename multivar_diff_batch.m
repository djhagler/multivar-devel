function multivar_diff_batch(instem,outstem,batchname,gender_flag,halves_flag)
%function multivar_diff_batch(instem,outstem,batchname,gender_flag,halves_flag)
%
...
outdir = [rootdir '/output/diff_batch'];
indir = [rootdir '/data_resid'];
hemilist = {'lh','rh'};
nbatches = 200;
forceflag = 0;
...
fname_mat = sprintf('%s/%s_info.mat',outdir,outstem);
if ~exist(fname_mat,'file') || forceflag
  fprintf('%s: saving info...\n',mfilename);
...
for j=1:nbatches
  ...
  fprintf(fid,'multivar_surf_diff(''%s'',''%s'',%d,%d,''%s'',''%s'',%d,%d);\n',...
    indir,outdir,p,q,instem,outstem,gender_flag,halves_flag);
  fprintf(fid,'exit;\n');
  fclose(fid);
  % add to list
