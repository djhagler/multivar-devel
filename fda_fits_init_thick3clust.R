...
outfix = "init"
infix = "resid"
instem_data_list = c("thick_3clust_resid")
pwidth = 3.5
pheight = 2.4
...
# plot colors
cols =     c("blue4",         "green",      "indianred4")
colshade = c("lightskyblue1", "lightgreen", "indianred")
...
#		save.image(file=datafile)
	...
	## plots with initial spline estimate subtracted from fit
	full_outfix = outfix
	if (legendflag == TRUE) {
	...
	for(jj in 1:length(files)){  
	  ...
	  ## new ranges
	  range.mat <- matrix(unlist(sp.cis[[jj]]), ncol = length(data)*4)
	  ## y range, or otherwise, do not include age into finding range...age(xaxis) is the 4th variable from each list of computed splines from sp.cis
	  range.vars = 1:length(range.mat[1,]) %% 4 !=0
	  range.mat = range.mat[,range.vars]
	  ## subtract range.mat by initial value of each spline fit, which is every 3rd column now
	  inits.loc = 1:length(range.mat[1,]) %% 3 ==1
	  inits = range.mat[1,inits.loc]
	  range.mat.shift = range.mat
	  for(ii in 1:length(data)){
	    range.mat.shift[,((ii*3-2):(ii*3))] =  range.mat[,((ii*3-2):(ii*3))]-inits[ii]
	  }
	  newrange = range(range.mat.shift)
	  ...
	  if (labelflag == TRUE) {
		...
		plot(data[,1],data[,2], xlab = "age", ylab = file, main = file, xlim = xrange, ylim = newrange, type = "n")
	  } else {
	    par(oma = c(0,0,0,0), mar = c(2,2,1,1))
		plot(data[,1],data[,2], xlab = "", ylab = "", main = "", xlim = xrange, ylim = newrange, type = "n")
	  }
	  for(ii in 1:length(data)){
	    ## confidence interval: for shaded region
	    sp = sp.cis[[jj]][[ii]]
	    init = sp$main.curve[1]
	    polygon(c(rev(sp$x), sp$x), c(rev(sp$upper.ci - init), sp$lower.ci - init), col = t_col(colshade[ii]), border = NA)
	    lines(x=sp$x , y=sp$main.curve - init, lwd = 2, col = t_col(cols[ii], percent = 20))
	  }
	  if (legendflag == TRUE) {
